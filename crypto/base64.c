...
ICACHE_FLASH_ATTR int base64_decode(const uint8_t *enc, size_t elen,
    ...
    for (i=0; i<elen; i++)
      if (!((65 <= enc[i] && enc[i] <= 90)  ||
            ...
            enc[i] == '+' || enc[i] == '/' || enc[i] == '='))
      {
          log_error("main", "bad input");
          return -2;
      }

    if (elen/4*3 > dlen) {
        log_error("main", "dec overflow");
        return -1;
    }
...
ICACHE_FLASH_ATTR int base64_encode(const uint8_t *dec, size_t dlen,
                                    uint8_t *enc, size_t elen) {
    size_t i, j;
    uint32_t v;

    if (elen < (dlen/3 + (dlen%3>0 ? 1 : 0)) * 4) {
        log_error("main", "enc overflow");
        return -1;
    }
